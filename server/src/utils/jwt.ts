...
export const signAccessToken = ( payload: Pick<TokenPayload, 'userId' | 'role'> & { exp?: number }, options?: SignerOptions ) => {
    const { exp } = payload
...
export const signRefreshToken = ( payload: Pick<TokenPayload, 'userId' | 'role'> & { exp?: number }, options?: SignerOptions ) => {
    const { exp } = payload
    const optionSigner: Partial<SignerOptions & { key: string | Buffer | PrivateKey }> = exp ? 
        {
